...
oldbp = ""
oldrp = ""
bot = discord.Bot(intents=intents, status=discord.Status.online, activity=discord.Activity(type=discord.ActivityType.watching, name="bunnies"))
...
async def premiumbunnies():
    ...
    if vp == 164:
        vp = 0
    number = random.randint(0, 163)
    while True:
        async with aiohttp.ClientSession() as cs:
            async with cs.get(f"https://bunnies.media/poster/{number}.png", headers={'User-agent': 'Just getting some rabbits'}) as r:
                res = await r.json()
                global oldbp
                global oldrp
                ...
                embed.set_footer(text=f"Taken from https://bunnies.media/poster/{number}.png")
                break
